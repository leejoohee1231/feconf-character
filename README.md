...
- 🚶 **코딩하면 걸어다님** — `/activity`(코딩/타이핑) 감지 중엔 화면 **반대 모서리로 걸어가고**(뒤뚱뒤뚱 + 발 번갈아), 멈추면 원래 자리로 돌아옴. `lingerMs`(기본 6초) 동안 활동이 없으면 귀가
...
| `walking`  | `/activity`(코딩) 이동 중 | 뒤뚱뒤뚱 · 발 번갈아 · 방향 시선 |
| `working`  | 반대 모서리 도착·집중     | 실눈 집중 · `...`        |
